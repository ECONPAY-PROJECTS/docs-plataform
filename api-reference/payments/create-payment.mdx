...
auth: "bearer"
description: "Criar um novo pagamento (PIX, Cartão de Crédito, Débito ou Boleto)"
---

## Descrição

Endpoint unificado para processar pagamentos. Suporta múltiplos métodos de pagamento:
...
<ParamField body="payment" type="object" required>
  ...
  <Expandable title="properties">
    ...
    <ParamField body="payment.installments" type="number" required>
      Número de parcelas (1 para à vista, até 12 para crédito)
    </ParamField>
    
    <ParamField body="payment.credit_card" type="object">
      Dados do cartão (obrigatório para `credit` e `debit`)
      <Expandable title="properties">
        <ParamField body="payment.credit_card.number" type="string" required>
          Número do cartão (apenas números)
        </ParamField>
        
        <ParamField body="payment.credit_card.holder_name" type="string" required>
          Nome impresso no cartão (MAIÚSCULAS)
        </ParamField>
        
        <ParamField body="payment.credit_card.exp_month" type="number" required>
          Mês de expiração (1-12)
        </ParamField>
        
        <ParamField body="payment.credit_card.exp_year" type="number" required>
          Ano de expiração (YYYY, ex: 2025)
        </ParamField>
        
        <ParamField body="payment.credit_card.cvv" type="string" required>
...
<Tabs>
  ...
  <Tab title="Cartão de Crédito">
    ...
    **Campos obrigatórios:**
    ...
    - `payment.credit_card.exp_month` (1-12)
    - `payment.credit_card.exp_year` (YYYY)
    - `payment.credit_card.cvv`
    
    **Status:**
    - `APPROVED`: Aprovado imediatamente
    - `FAILED`: Recusado (webhook enviado)
  </Tab>
  
  <Tab title="Cartão de Débito">
    ### Cartão de Débito
    
    - Apenas à vista (installments: 1)
    - Aprovação rápida
